...
public class VoxelGrid : Node2D
{
    ...
    {	
		sharpFeatureLimit = Mathf.Cos(Mathf.Deg2Rad(maxFeatureAngle));
    ...
    private void SetVoxelColors() {
        for (int i = 0; i < voxels.Length; i++) {
            voxelMeshes[i].SelfModulate = voxels[i].state ? Colors.Black : Colors.White;
        }
    }
	...
	private static Vector2 ComputeIntersection (Vector2 p1, Vector2 n1, Vector2 p2, Vector2 n2) {
		...
		float u2 = -n1.Dot(p2 - p1) / n1.Dot(d2);
		return p2 + d2 * u2;
	}
